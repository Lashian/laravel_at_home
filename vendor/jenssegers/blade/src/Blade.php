...
class Blade implements FactoryContract
{
    ...
    {
        $absoluteViewPath = $_SERVER['DOCUMENT_ROOT'].'/PHP_plain_project_done/app/views';
        $absoluteCachePath = $_SERVER['DOCUMENT_ROOT'].'/PHP_plain_project_done/app/views/cache';

        $viewPaths = str_replace('\\', '/', $absoluteViewPath);
        $cachePath = str_replace('\\', '/', $absoluteCachePath);
